...
# This script uses a metadata table (metadata/metadata.txt) that has the columns:
...
# Read files need to start with the SRA accession number
# Requieres python and fastap

SPECIES_DIR=
THREADS=1 # change this number if necessary

cd  ${SPECIES_DIR}/
mkdir  clean_reads fastpReports

sample_info=($(awk -F '\t' -v r=$SLURM_ARRAY_TASK_ID 'NR == r {print $0}' $PWD/metadata/metadata.txt))
length_required=$( ${sample_info[4]} == "short"  && echo 30 || echo 50)
 
#This allows us to find reads regardless of their origin (NGDC, SRA or ENA) 
MYREADS=($(ls $PWD/RAW_DATA/${sample_info[0]}*))
